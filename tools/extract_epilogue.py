...
def main():
    found = []
    for p in PACKS:
        path = os.path.join(SR, p, "data", "misc", "epilogue.bytes")
    ...
    for pack, text, paras in found:
        ...
        for key in ("bark_" + md5_16(text), "bark_" + md5_16(collapse(text))):
            if key in data:
                ...
                data[key] = dict(entry)
                added += 1
        print(f"[{GAME}] {pack}: epilogue, {len(paras)} paragraph(s), {len(text)} chars")
